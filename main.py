...
def train(**kwargs):
    ...
    dataset = Data.TensorDataset(data_tensor=torch.Tensor(X), target_tensor=torch.Tensor(Y))
    loader = Data.DataLoader(dataset=dataset, batch_size=options.batch_size, shuffle=True, drop_last=True, num_workers=options.core)
